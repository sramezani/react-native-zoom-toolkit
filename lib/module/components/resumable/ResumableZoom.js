...
const ResumableZoom = props => {
  ...
  const pan = Gesture.Pan().withTestId('pan').enabled(panEnabled && gesturesEnabled && enablePanGestureByDoubleTap).maxPointers(1).onStart(onPanStart).onChange(onPanChange).onEnd(onPanEnd);
  ...
  const doubleTap = Gesture.Tap().withTestId('doubleTap').enabled(tapsEnabled && gesturesEnabled).maxDuration(250).numberOfTaps(2).onStart(onDoubleTapStart).onEnd(onDoubleTapEnd);
